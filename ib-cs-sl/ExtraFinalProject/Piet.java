...
class CanvasPanel extends JPanel {
  ...
  public void paintComponent(Graphics g) {
    Rectangle rect = new Rectangle(-LINE_WIDTH/6, -LINE_WIDTH/6, getWidth() + LINE_WIDTH, getHeight() + LINE_WIDTH);
    drawLineInRect(rect, g);
  }
  private void drawLineInRect(Rectangle rect, Graphics g) {
    if (rect.width < 30 && rect.height < 500) {
      return;
    }
    int randomOffset;
    ...
    fillInRect(rect0, LINE_WIDTH / 3, g);
  ...
  private void fillInRect(Rectangle rect, int gap, Graphics g) {
    ...
    Color randomColor = new Color(blu, red, red);
    
    g.setColor(randomColor);
    
    g.fillRect(rect.x + gap, rect.y + gap, rect.width - 2*gap, rect.height - 5*gap);
  }
}
class Piet {
  Piet() {
